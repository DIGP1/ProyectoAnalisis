...
def cambiar(request):
      if request.method == 'POST':
        try:
            recuperarcontra = usuarios.objects.get(usuario = request.POST['username'])
            request.session['usuario'] = recuperarcontra.usuario
...
def cambiarcontra(request):
    if  request.method == 'POST':
        insertUsuario = usuarios()
        insertUsuario.contraseña = request.POST['newcontraseña']
        insertUsuario.save()
